...
class BankTransfer {
  ...
  // Він приймає суму переказу як параметр
   initiateTransfer(amount) {
    const calculatedAmount = this.calculateFee(amount); // Для ініціювання банківського переказу спершу обчислюється сума з урахуванням комісії calculatedAmount = this.calculateFee(amount)
    console.log(`Ініціюємо банківський переказ: $${calculatedAmount}`); // Виводимо інформацію про ініціювання банківського переказу Ініціюємо банківський переказ: $${calculatedAmount}
  }
  ...
  // Він приймає amount переказу як параметр та повертає число після розрахування комісії
   calculateFee(amount) {
    return amount * 1.02; // Логіка розрахунку комісії за переказ amount * 1.02
  }
}
...
class TransferAdapter {
  ...
  // Робимо метод initiateTransfer, який адаптує API WalletTransfer до API BankTransfer.
   ...
   initiateTransfer(amount) {
    const calculatedAmount = this.calculateFee(amount); // Викликаємо допоміжний метод calculateFee для обчислення комісії за переказ та результат записуємо в константу calculatedAmount
    this.transferSystem.processTransfer(calculatedAmount); // Викликаємо метод processTransfer об'єкту WalletTransfer з calculatedAmount.
    // В результаті повертаємо результат виконання переказу.
  }

  calculateFee(amount) {
    return amount * 1.2; // Створюємо метод calculateFee, що приймає amount та обчислює суму комісії за переказ amount * 1.2, засновуючись на вхідній сумі.
  }
}
